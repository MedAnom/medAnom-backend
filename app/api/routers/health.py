...
from app.api.deps import get_current_user  # 기존 인증 의존성
...
def analyze_health(payload: Input,
                   me: User = Depends(get_current_user),
                   session: Session = Depends(get_session)):
    if not _models: raise HTTPException(503, "모델 준비 중")
    age_g = age_to_group(payload.AGE)
    x = np.array([payload.SEX, age_g, payload.HGB, payload.TCHOL, payload.TG, payload.HDL], dtype=float).reshape(1,-1)

    risks = {}
    for t in TARGETS:
        p1 = float(_models[t].predict_proba(x)[0,1])
        thr = _thresholds.get(t, 0.5)
        risks[t] = {"prob": round(p1,4), "label": int(p1>=thr), "thr": thr}
    ...
    rec = HealthRecord(
        user_id=str(me.id),
        sex=int(payload.SEX), age=int(payload.AGE),
        ...
        smoking=payload.smoking, alcohol_per_week=payload.alcohol_per_week, stress=payload.stress,
        prob_ane=risks["ANE"]["prob"], prob_ihd=risks["IHD"]["prob"], prob_stk=risks["STK"]["prob"],
        label_ane=risks["ANE"]["label"], label_ihd=risks["IHD"]["label"], label_stk=risks["STK"]["label"],
        health_score=health_score, summary_text=summary_text
    )
...
def recent_history(limit: int = Query(30, ge=1, le=200),
                   me: User = Depends(get_current_user),
                   session: Session = Depends(get_session)):
    q = select(HealthRecord).where(HealthRecord.user_id==str(me.id)).order_by(HealthRecord.created_at.desc()).limit(limit)
    rows: List[HealthRecord] = session.exec(q).all()
    return [
        {
            "id": r.id, "created_at": r.created_at.isoformat(),
            "sex": r.sex, "age": r.age, "hgb": r.hgb, "tchol": r.tchol, "tg": r.tg, "hdl": r.hdl,
            "prob": {"ANE": r.prob_ane, "IHD": r.prob_ihd, "STK": r.prob_stk},
            "label": {"ANE": r.label_ane, "IHD": r.label_ihd, "STK": r.label_stk},
            "health_score": r.health_score, "summary_text": r.summary_text
        } for r in rows
    ]
